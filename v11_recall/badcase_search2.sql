...
ObjPass AS (
    SELECT tw.turn_start, tw.turn_end, d.obj_id, d.ts,
           ...
           d.x AS ego_x, e.speed AS ego_speed,
...
ObjResid AS (
    SELECT op.turn_start, op.turn_end, op.obj_id, op.ts, op.dist, op.dr_speed,
           op.h_rel_abs, op.fx, op.fy, op.ego_x, op.ego_speed,
...
ObjAgg AS (
    SELECT turn_start, turn_end, obj_id,
           ...
           SUM(CASE WHEN ego_speed < 1.5 AND dist < 40 AND ego_x > 2 THEN 1 ELSE 0 END) AS n_ego_stopped
    FROM ObjResid
    ...
    HAVING COUNT(DISTINCT ts) >= 2
       ...
       AND SUM(CASE WHEN ego_speed < 1.5 AND dist < 40 AND ego_x > 2 THEN 1 ELSE 0 END) >= 1
)
SELECT turn_start, turn_end, obj_id, n_frames, max_speed, min_dist, min_resid, diff_init, n_ego_stopped
FROM ObjAgg
ORDER BY min_resid
