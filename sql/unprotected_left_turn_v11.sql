...
-- ==============================================================================
-- v11 残余冲突分支（新增）：红灯排队等左转，对向末班直行车在等灯期通过，
-- 其通行位置落在 ego 之后实际左转路径上。
-- 机理（实证 14GHqPsrhUgo1qw3VOzuqK202606）：等灯期在 Intersection∩Turning
-- 窗口之前，v10.4 锚点层漏掉；且 obj 通行与 ego 穿过交叉点时间差 10-16s，
-- |Δts|<=5s 同时刻交叉判据也过不了。
-- 判据：ego 在 obj 通行时刻静止 + obj 原路对向直行 + obj 通行点落在 ego
--       之后实际行驶路径 <5m（固定系，锚定转弯起点姿态）
-- ==============================================================================
ResidTurn AS (
...
residual_final AS (
    ...
    FROM ResidEvents r
    WHERE NOT EXISTS (
        SELECT 1 FROM merged_events me
        WHERE me.start_ts <= r.turn_end AND me.end_ts >= r.turn_start
    )
      AND r.frame_count >= 2
),
main_final AS (
...
SELECT * FROM (
    SELECT * FROM main_final
    UNION ALL
    SELECT * FROM residual_final
) u
WHERE u.junction_type IS NULL OR u.junction_type NOT IN ('t_junction', 'small_t_junction')
ORDER BY u.start_ts;
